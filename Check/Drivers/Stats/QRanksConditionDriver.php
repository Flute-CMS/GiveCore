...
/**
 * Драйвер условия: проверка статистики QRanks
 */
class QRanksConditionDriver extends AbstractCheckDriver
{
    protected const MOD_KEY = 'QRanks';
    ...
    {
        return __('givecore.drivers.qranks.name');
    }

    public function description(): string
    {
        return __('givecore.drivers.qranks.description');
    }
    ...
    public function check(User $user, array $params = []): bool
    {
        ...
        $metric = $params['metric'] ?? 'rank_points';
        ...
        try {
            ...
            $tableName = $additional['table_name'] ?? 'qranks_stats';
            ...
            logs()->warning("RoleSync QRanks check failed for user {$user->id}: " . $e->getMessage());
    ...
    public function checkFields(): array
    {
        return [
            ...
            'metric' => [
                ...
                'required' => true,
                'options' => [
                    'rank_points' => __('givecore.metrics.score'),
    ...
    {
        $mappings = [
            'rank_points' => ['rank_points', 'points', 'score'],
            'kills' => ['kills'],
            'deaths' => ['deaths'],
