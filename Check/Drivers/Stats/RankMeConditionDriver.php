...
/**
 * Драйвер условия: проверка статистики RankMe
 */
class RankMeConditionDriver extends AbstractCheckDriver
{
    protected const MOD_KEY = 'RANKME';
    ...
    {
        return __('givecore.drivers.rankme.name');
    }

    public function description(): string
    {
        return __('givecore.drivers.rankme.description');
    }
    ...
    public function check(User $user, array $params = []): bool
    {
        ...
        try {
            ...
            $tableName = $additional['table_name'] ?? 'rankme';
            ...
            logs()->warning("RoleSync RankMe check failed for user {$user->id}: " . $e->getMessage());
    ...
    public function checkFields(): array
    {
        return [
            ...
            'metric' => [
                ...
                'required' => true,
                'options' => [
                    'score' => __('givecore.metrics.score'),
                    'kills' => __('givecore.metrics.kills'),
                    'deaths' => __('givecore.metrics.deaths'),
                    'headshots' => __('givecore.metrics.headshots'),
