...
class AmxConditionDriver extends AbstractCheckDriver
{
    ...
    {
        return __('givecore.drivers.amx_ban.name');
    }

    public function description(): string
    {
        return __('givecore.drivers.amx_ban.description');
    }
    ...
    public function check(User $user, array $params = []): bool
    {
        ...
        try {
            ...
            $bans = $db
                ...
                ->where(static function ($q) use ($steamId2, $steamIdShort) {
                    $q
                        ...
                        ->orWhere('player_id', 'VALVE_' . $steamIdShort)
                        ->orWhere('player_id', 'STEAM_' . $steamIdShort);
                })
                ->where('expired', 0)
                ->fetchAll();
            ...
            logs()->warning("RoleSync AMX ban check failed for user {$user->id}: " . $e->getMessage());
